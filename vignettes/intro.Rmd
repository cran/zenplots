...
author: "Marius Hofert and R.W. Oldford"
date: '`r Sys.Date()`'
output:
  html_vignette:
    toc: yes
...
has 15 nodes, here labelled "1" to "9", followed by "a" to "f" (to make plotting
easier).
...
index, "f",  complete its pairing with every other index before "e" completes
all of its pairings.  All of "e"'s pairings complete before those of "d", all of
"d"'s before "c", and so on until the last pairing of "1" and "2" are completed.
...
sequence.  A simple reversal, that is `rev(zenpath(15, method =
"back.loaded"))`,  would have them appear at the beginning of the sequence.  In
